...
class TestValidatedModelSchema(TestCase):
    ...
    def test_load_does_not_fail_dict_level_gt_0(self):
        incorrect_data_value = 'foobar'

        attrs['office_id'] = hash_key
        attrs['employees'][0]['office_location']['latitude'] = incorrect_data_value

        data, errors = OfficeSchema().load(attrs)

        self.assertDictEqual(errors, {})
        self.assertEqual(data.attribute_values['employees'][0]['office_location']['latitude'], incorrect_data_value)
